...
class TfTrainableModel(ApiModel):
    ...
    def __enter__(self):
        try:
            self._checkpoint_model_folder = Downloader.download_model(self._model_name)
            self._checkpoint_model_pipeline_file = Downloader.download_pipeline(self._model_name)
            #self._load_dataset()
            return self
        except  Exception as ex:
