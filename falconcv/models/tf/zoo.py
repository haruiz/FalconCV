...
from falconcv.util import FileUtil, LibUtil, ConfigUtil
...
class ModelZoo:
    ...
    def available_models(arch=None) -> []:
        try:
            models = {}
            r = requests.get(ConfigUtil.get_tfodapi_model_zoo_uri())
    ...
    def available_pipelines(cls):
        try:
            response = requests.get(ConfigUtil.get_tfodapi_model_config_uri())
            config_files = {}
    ...
    @classmethod
    @typeassert(model_name=str)
    def download_model(cls, model_name: str) -> Path:
